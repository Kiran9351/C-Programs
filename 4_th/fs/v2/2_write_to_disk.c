...
int count_written_bytes;
/*
void read_pointer_block(int fd, int fd2, int fd3, int block_no, int no_of_ints)
{
	...
	while(i < no_of_ints)
	{
		if(fs >= blocksize)
		{
			...
			fs = fs - blocksize;
		//	count_written_bytes += blocksize;
		}
		else
		{
			read(fd, buff, fs);
			write_data_block(fd, bit_arr[i], buff, fs);
		//	count_written_bytes += fs;
			fs = fs - fs;
		}
	
		//write_data_block(fd, bit_arr[i], buff, blocksize);
		
		//printf("fs = %d, count_written_bytes = %d\n", fs, count_written_bytes);
...
void read_parent(int fd, int fd2, int fd3, int block_no, int no_of_ints, int arr_index);

void read_child(int fd, int fd2, int fd3, int block_no, int arr_index)
{
        int no_of_ints = 0;
        ...
        if(arr_index == 0)
        {
                read_pointer_block(fd, fd2, fd3, block_no, no_of_ints);
        }
        else
        {
                read_parent(fd, fd2, fd3, block_no, no_of_ints, arr_index);
        }
...
void read_parent(int fd, int fd2, int fd3, int block_no, int no_of_ints, int arr_index)
{
        ...
        while(i < no_of_ints)
        {
                read_child(fd, fd2, fd3, iarr[i], arr_index - 1);
                i = i + 1;
        }
...
void create_file(int fd,char *source, int block_no, int no_of_ints, int arr_index)
{
	...
	fd3 = open("f1.txt",O_CREAT | O_RDWR, 00600);
...
void rd_pointer_block(int fd, int fd2, int fd3, int block_no, int no_of_ints)
{
        int i = 0, *bit_arr = NULL;
        unsigned char *buff = NULL;
...
void rd_parent(int fd, int fd2, int fd3, int block_no, int no_of_ints, int arr_index);

void rd_child(int fd, int fd2, int fd3, int block_no, int arr_index)
{
        int no_of_ints = 0;
        ...
        if(arr_index == 0)
        {
                rd_pointer_block(fd, fd2, fd3, block_no, no_of_ints);
        }
        else
        {
                rd_parent(fd, fd2, fd3, block_no, no_of_ints, arr_index);
        }
...
void rd_parent(int fd, int fd2, int fd3, int block_no, int no_of_ints, int arr_index)
{
        ...
        while(i < no_of_ints)
        {
                rd_child(fd, fd2, fd3, iarr[i], arr_index - 1);
                i = i + 1;
        }
...
void rd_file(int fd, char* dest, int block_no, int no_of_ints, int arr_index)
{
	...
	fd2 = open(dest, O_CREAT | O_RDWR, 00600);
	int fd3 = open("f2.txt", O_CREAT | O_RDWR, 00600);
	rd_parent(fd, fd2, fd3, block_no, no_of_ints, arr_index);
